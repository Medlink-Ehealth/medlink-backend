 ...
 */
const getOffsetTimestamp = (interval: string | number) => {
  if (!interval) return "'Interval' must be defined";
  //stringify interval
  interval = `${interval}`;
  ...
  if (!isNaN((interval as unknown as number) * 1))
    interval = (interval as unknown as number) * 1;
  ...
  if (typeof interval !== "number") {
    ...
    const digit = (interval.split(suffix)[0] as unknown as number) * 1;
    if (typeof digit === "number") {
      if (["m", "h", "d"].includes(suffix)) {
        timestamp = setTimeInPast
          ? currentTimestamp - digit * period[suffix as keyof typeof period]
          : currentTimestamp + digit * period[suffix as keyof typeof period];
      } else {
        timestamp = setTimeInPast
