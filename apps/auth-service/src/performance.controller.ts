...
const isRedis = async (): Promise<Redis | null> =>
  await new Promise((resolve, reject) => {
    ...
    if (redisConnect) {
      if (typeof redisConnect === "string")
        return reject(new Error(redisConnect));
      ...
      const redis = new Redis({
        ...
        .on("error", (err) => {
          ...
          const quitCheck =
            two000Count > 3 || process.env.NODE_ENV !== "production";
          if (quitCheck) {
            logger.warn(
              "Unable to initiate a successful connect to a redis server and has been halted!",
            );
