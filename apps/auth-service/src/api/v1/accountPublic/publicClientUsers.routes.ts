...
router.post(
  ...
  async (ctx, next) => {
    ...
    if (!ctx.sequelizeInstance) {
      logger.error(
        "No active sequelizeInstance detected, and unable to run DB related entries in publicClientUsers route",
      );
        ...
        if (checkVerified) {
          ...
          const identifier =
            ...
            checkVerified.dataValues.phoneNumber
              ...
              : checkVerified.dataValues.email
                ? 1
                : 3; // 1: email, 2: both, 3: phoneNumber
  ...
  async (ctx) => {
    ...
    if (ctx.state.newUser) {
      ...
      ctx.message =
        ...
        "Oops! Apologies we are currently unable to sign you up but we are working on it.";
      return;
    }
  },
);
  ...
  async (ctx) => {
    ...
    try {
      ...
      if (
        ...
        ctx.status = statusCodes.BAD_REQUEST;
        ctx.message =
          "Password reset OTP is invalid or expired. Kindly generate a new code";
        return;
      }
      ...
      const whereFilter = {
        [emailOrPhone.includes("@") ? "email" : "phoneNumber"]: emailOrPhone,
      };

      const user = await Client(ctx.sequelizeInstance!).update(
