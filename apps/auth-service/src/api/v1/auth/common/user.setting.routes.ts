...
router.post("/2fa/:action", requestParser(), async (ctx) => {
  try {
    const twoFAaction = ctx.params.action
      ? ctx.params.action.toLowerCase()
      : undefined; // enable|disable
    if (
      !twoFAaction ||
      (twoFAaction && !["enable", "disable"].includes(twoFAaction))
    ) {
      ctx.status = statusCodes.NOT_ACCEPTABLE;
      ctx.message =
        "Unrecognisable 2FA action. Please define the ACTION you intend to take: either 'enable' or 'disable'";
      return;
    }
    const passcode =
      ctx.request.body && (ctx.request.body as JsonObject).passcode; //required if ACTION is 'disable'
    ...
    if (
      ...
      twoFAaction === "disable" &&
      (!twoFAstatus || (twoFAstatus && !twoFAstatus["verified"]))
    ) {
    ...
    if (twoFAaction === "disable") {
      if (!passcode) {
        ...
        ctx.message =
          "Define a passcode from an authenticator APP to disable 2FA on this account";
        return;
      }
      ...
      if (validate === true) {
        ...
        const accesssTokenLifetime =
          config.authTokenLifetime && !isNaN(Number(config.authTokenLifetime))
        ...
        if (!user["secured"])
          ctx.sequelizeInstance!.models[
            userTypes[ctx.state.user.type as "client"]
          ].update({ secure: false }, { where: { uuid: user.uuid } });
        ...
        notificationLogger({
          ...
          meta: {
            target: userTypes[ctx.state.user.type as "client"] as "Admin",
            uuid: "self",
          },
        });
        ...
        return (ctx.body = {
          status: statusCodes.OK,
          statusText:
            "2FA disable successfully. Replace existing access token with new one attached",
          //acccount: user,
          token: token,
        });
      } else {
        ...
        ctx.message =
          "An error occurred while validating passcode code. Please try again later";
        return;
      }
    } else {
      ...
      const userSecret =
        ...
        checkPreviousSecretTimeout > Date.now() - 1000 * 15
      ...
      if (typeof userSecret === "string") {
        ...
        return (ctx.body = {
          ...
          statusText:
            "Scan/paste code in an authenticator App to continue. Valid for 30 seconds",
        ...
        ctx.status = statusCodes.INTERNAL_SERVER_ERROR;
        ctx.message =
          "An error occurred while activating 2FA. Please try again later";
        return;
      }
    }
  } catch (err) {
...
router.post("/2fa/enable/confirm", requestParser(), async (ctx) => {
  try {
    ...
    if (!passcode) {
      ctx.status = statusCodes.BAD_REQUEST;
      ctx.message =
        "Define a passcode from an authenticator APP to confirm 2FA on this account";
      return;
    }
    const user = ctx.state.user;
    let security: {
      ...
      [x: string]:
        | undefined
        | { verified: boolean; [secret: string]: string | boolean | number };
    } = {};
    ...
    if (security && security["2fa"]) {
      if (!security["2fa"]["secret"]) {
        ctx.status = statusCodes.BAD_REQUEST;
        ctx.message =
          "You do not seem to have 2FA enabled on your account. Please try enabling 2FA";
        return;
      } else {
        ...
        if (validate === true) {
          ...
          if (!ctx.state.user["secured"] || user["secured"]) {
            ...
            ctx.sequelizeInstance!.models[
              userTypes[ctx.state.user.type as "client"]
            ].update({ secured: true }, { where: { uuid: user.uuid } });
          }
          //Create notification
          notificationLogger({
            detail:
              "Two Factor Authentication (2FA) is now enabled on your account, and makes access to your account more secured.",
            meta: {
              target: userTypes[ctx.state.user.type as "client"] as "Admin",
              uuid: "self",
            },
          ...
          return (ctx.body = {
            status: statusCodes.OK,
            statusText:
              "Two factor authentication activated on your account. Replace access token with new one attached",
            //acccount: user,
            token: token,
          });
        } else {
          logger.error(
            "2FA validation error during 2FA enabling confirmation: ",
            validate,
          );
          ctx.status = statusCodes.BAD_REQUEST;
          ctx.message =
            "Oops! We are unable to verify your code. You may need to restart the authentication process to resolve this.";
          return;
        }
      }
    }
...
router.post("/recovery-email/:action", requestParser(), async (ctx) => {
  try {
    const emailAction = ctx.params.action
      ? ctx.params.action.toLowerCase()
      : undefined; // enable|disable
    if (
      !emailAction ||
      (emailAction && !["add", "remove"].includes(emailAction))
    ) {
      ctx.status = statusCodes.NOT_ACCEPTABLE;
      ctx.message =
        "Unrecognisable recovery email action. Please define the ACTION you intend to take: either 'add' or 'remove'";
      return;
    }
    const email =
      ctx.request.body && ((ctx.request.body as JsonObject).email as string);
    if (!email || (email && !validator.isEmail(email))) {
      ctx.status = statusCodes.BAD_REQUEST;
      ctx.message =
        "No valid email provided to add or remove from recovery emails";
      return;
    }
    const user = ctx.state.user;
    const otpPresent: string | undefined = (ctx.request.body as JsonObject)
      .otp as string;
    let security: {
    ...
    const existingEmails: { verified: boolean; email: string }[] =
      security && security["recovery_emails"]
        ? security["recovery_emails"]
        : [];
    ...
    if (emailAction === "add" && existingVerifiedEmails.includes(email)) {
      ctx.status = statusCodes.NOT_MODIFIED;
      ctx.message =
        "This email is already added as a recovery email on this account";
      return;
    } else if (
      emailAction === "remove" &&
      !existingVerifiedEmails.includes(email)
    ) {
      ctx.status = statusCodes.NOT_MODIFIED;
      ctx.message =
        "This email is not added as a recovery email on this account";
      return;
    }
    ...
    if (!otpPresent) {
      ...
      if (OTPvalue)
        ...
        ctx.status = statusCodes.INTERNAL_SERVER_ERROR;
        ctx.message =
          "An error occurred while sending verification code to your email. Please try again later";
        return;
      }
      ...
      try {
        ...
        ctx.status = statusCodes.INTERNAL_SERVER_ERROR;
        ctx.message =
          "Server currently unable to verify OTP. Please try again.";
        return;
      }
    }
  } catch (err) {
