...
router.get("/me", async (ctx) => {
  if (ctx.isAuthenticated()) {
    ...
    const security = await UserSecurity(ctx.sequelizeInstance!).findByPk(
      ctx.state.user.uuid,
    );
    ...
    return (ctx.body = {
      ...
      account: {
        ...user.dataValues,
        auth_features: ctx.state.user["auth_features"],
      },
    });
  }
...
router.patch(
  ...
  async (ctx, next) => {
    if (ctx.state.user.type === "client")
      await clientFormValidator.updateAccount(ctx, next);
    else if (ctx.state.user.type === "admin")
      await adminFormValidator.updateAccount(ctx, next);
    else {
      ctx.status = statusCodes.UNAUTHORIZED;
  ...
  async (ctx, next) => {
    ...
    //call next
    await updateAccount({
      userType: userTypes[ctx.state.user.type as "client"],
    })(ctx, next);
  },
  (ctx) => {
        ...
        if (user) {
          if (user.getDataValue("avatar"))
            try {
              ...
              logger.error(
                `${userTypes[ctx.state.user.type as "client"]} Avatar unlinking error`,
                err,
              );
            }
          ...
          if (ctx.request.body["avatar"] === null)
...
router.patch(
  ...
  async (ctx, next) => {
    if (ctx.params["passwordOrEmail"].toLowerCase() !== "password")
      await next();
    // process password changes
    else {
      // lets fetch the current, then compare the new and repeated password
      const { currentPassword, newPassword, repeatedNewPassword, otp } = ctx
            ...
            if (otp) {
              ...
              if (
                ...
                (typeof identifiers === "string" &&
                  identifiers !== verifiedID) ||
                !identifiers.includes(verifiedID)
              ) {
                ctx.status = statusCodes.NOT_ACCEPTABLE;
                ctx.message =
                  "The OTP code provided seems expired. Kindly request another";
                return;
              }
              // if all is good, effect password changes
              await user.update({
                password: hashPassword(newPassword as string),
              });
              ctx.status = statusCodes.OK;
              ...
              if (OTPvalue) {
                const userIDs = Array.isArray(identifiers)
                  ? identifiers
                  : [identifiers];
  ...
  async (ctx) => {
    ...
    if (!email) {
      ...
      if (
        !user ||
        !comparePassword(password as string, user.getDataValue("password"))
      ) {
        ctx.status = statusCodes.BAD_REQUEST;
        ctx.message = "Incorrect password";
        return;
      ...
      if (
        updateType === "email" &&
        user.getDataValue("email") === updatedItem
      ) {
        ctx.status = statusCodes.CONFLICT;
        ctx.message =
          "You are using the same existing email. Provide a different email to change email address.";
        return;
      }
    }
    ...
    try {
      ...
      if (otp) {
        try {
          const verifiedID = await otpVerifier(
