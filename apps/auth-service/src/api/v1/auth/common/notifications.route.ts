  ...
  async (ctx) => {
    ...
    if (ctx.state.dbQuerier) {
      const queryFilter = () => {
        ...
        let reservedMeta;
        if (!ctx.state.dbQuerier.where)
          ctx.state.dbQuerier.where = { meta: {} };
        else if (!ctx.state.dbQuerier.where.meta)
          ctx.state.dbQuerier.where["meta"] = {};
        ...
        ctx.state.dbQuerier.where.meta = {
          [Op.and]: [
            { [Op.contains]: { target: { type: "User" } } },
            { [Op.contains]: { target: { uuid: ctx.state.user.uuid } } },
          ],
        };
      ...
      if (
        !ctx.state.dbQuerier.where ||
        (ctx.state.dbQuerier.where && !ctx.state.dbQuerier.where.meta)
      ) {
        queryFilter();
      }
      ...
      if (
        ctx.state.dbQuerier.where.meta &&
        typeof ctx.state.dbQuerier.where.meta === "object"
      ) {
        ...
        ctx.message =
          "Define at least a META filter to fetch notifications. Or leave out META filter to filter to server default";
      }
    }
  },
);
...
router.get("/:uuid", async (ctx) => {
  ...
  const notification = await ctx.sequelizeInstance!.transaction(async (t) => {
    ...
    if (highestRole && ctx.state.user.role >= highestRole.dataValues.level) {
      return await Notification(ctx.sequelizeInstance!).findByPk(
        ctx.params.uuid,
        { transaction: t },
      );
    } else return false;
  ...
  if (notification) {
    ...
    ctx.status = statusCodes.NOT_FOUND;
    ctx.message =
      "Oops. We are unsure you have the permission to view the notification you are looking for!";
    return;
  }
});
...
router.get("/:uuid/read", async (ctx) => {
  //Only the highest user role besides the DEV role should be able to see a notification
  const notification = await Notification(ctx.sequelizeInstance!).update(
    { status: "read" },
