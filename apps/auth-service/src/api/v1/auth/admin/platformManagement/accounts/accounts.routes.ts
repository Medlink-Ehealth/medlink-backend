  ...
  async (ctx, next) => {
    if (
      ...
      (ctx.method.toLowerCase() === "get" &&
        ctx.state.user.role < 2 &&
        ctx.path.includes("/suspend/"))
    ) {
      ctx.status = statusCodes.UNAUTHORIZED;
      ctx.message =
        "Elevated operations permission is required to create/modify user accounts";
      return;
    } else await next();
  },
...
router.get(
  ...
  async (ctx, next) => {
    ...
    if (ctx.params["usersType"] === "{userType}")
      delete ctx.params["usersType"];
    ...
    if (typeof importedUserType === "string" && importedUserType.includes(","))
      importedUserType = importedUserType.split(",");

    const usersType = importedUserType
      ? typeof importedUserType === "string" &&
        userTypesModelMap[importedUserType as "admin"]
        ? [importedUserType]
        : Array.isArray(importedUserType) &&
            userTypesModelMap[importedUserType[0] as "admin"] // only checking that 1st item in query array exists in prefined types mapper
          ? importedUserType
          : undefined
        ...
        const users = await Promise.all(
          usersType.map(async (type) => {
            if (type) {
              ...
              const typeModel = (
                type === "admin" || type === "admins" ? Admin : Client
              ) as (db: Sequelize) => ModelStatic<AdminStatic | ClientStatic>;
        ...
        users.forEach((userGroup) => {
          if (userGroup && userGroup.length)
            userGroup.forEach(async (account) => {
              ...
              getTimestamps.push(
                UserAccessTimestamp(ctx.sequelizeInstance!).findByPk(
                  account.dataValues.uuid,
                  {
                    transaction: t,
                  },
                ),
            ...
            if (!userTypesModelMap[usersType[index] as "admin"]) return;
              ...
              groupUsersByType[key] = usersGroup.map((account) => {
                ...
                if (UsersAccessTimestamps[thisAccount.uuid]) {
                  (thisAccount as unknown as Record<string, unknown>)[
                    "access"
  ...
  async (ctx, next) => {
    ...
    if (!validator.isEmail(email)) {
      throwError(
        ...
        "Unable to resolve a valid account email.",
      );
    }
    const thisAccount = await ctx.sequelizeInstance!.transaction(async (t) => {
      const thisAccountProfile = await ctx
      ...
      if (thisAccountProfile) {
        const accessTimestamps = await UserAccessTimestamp(
          ctx.sequelizeInstance!,
        ).findByPk(thisAccountProfile.dataValues.uuid, {
          transaction: t,
        });
...
router.post(
  ...
  async (ctx) => {
    if (ctx.state.newUser) {
      ...
      await UserSetting(ctx.sequelizeInstance!).create({
        user_uuid: ctx.state.newUser.dataValues.uuid,
        user_type: "admin",
      });
      const profileData = {
...
router.get(
  ["/user/:email/action/:action", "/:usersType/:email/action/:action"],
  async (ctx) => {
    const usersType = ctx.params["usersType"] || ctx.query["type"];
    const email = ctx.params["email"];
    ...
    try {
      if (validator.isEmail(email)) {
        const thisAccount = await ctx.sequelizeInstance!.models[
          userTypesModelMap[usersType as "admin"]
        ].findOne({
          where: { email: email },
        });
        if (thisAccount) {
          if (
            thisAccount instanceof Admin(ctx.sequelizeInstance!) &&
            thisAccount.dataValues.role === 999
          ) {
          ...
          if (thisAccount instanceof Admin(ctx.sequelizeInstance!)) {
            ...
            if (roles) {
              const appRoles = roles.sort(
                (a, b) => b.dataValues.level - a.dataValues.level,
              );
              let managerialRole;
              if (appRoles[0].dataValues.level === 999)
                managerialRole = appRoles[1].dataValues;
              //a double check
              else if (appRoles[appRoles.length - 1].dataValues.level === 999)
                managerialRole = appRoles[appRoles.length - 2].dataValues;

              if (
                managerialRole &&
                thisAccount.dataValues.role === managerialRole.level &&
          ...
          if (ctx.params.action === "activate") {
            ...
            if (
              thisAccount instanceof Admin(ctx.sequelizeInstance!) &&
              thisAccount.dataValues.role === 0
            )
              updates = { ...updates, role: 1 };
  ...
  try {
    if (validator.isEmail(email)) {
      ...
      if (thisAccount) {
        ...
        if (role) {
          const checkRoleExists = await AdminRole(
            ctx.sequelizeInstance!,
          ).findOne({
            where: { level: role },
          });
          if (checkRoleExists) {
            const update: { state?: boolean; role: number } = {
              role: role as number,
            };
            ...
            ctx.status = statusCodes.NOT_ACCEPTABLE;
            ctx.message =
              "The role defined does not exist. Please try a different role.";
            return;
          }
          ...
          if (thisAccount instanceof Admin(ctx.sequelizeInstance!)) {
            ...
            if (roles) {
              ...
              if (
                ...
                ctx.status = statusCodes.NOT_ACCEPTABLE;
                ctx.message =
                  "Managerial roles are preserved from deletion. Downgrade this account role to allow deletion.";
                return;
              }
            }
          }
