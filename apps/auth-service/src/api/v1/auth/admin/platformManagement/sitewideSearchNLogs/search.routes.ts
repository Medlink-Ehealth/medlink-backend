...
router.use(requestParser({ multipart: true }), async (ctx, next) => {
  const search =
    (ctx.request.body && (ctx.request.body as JsonObject).search) ||
    ctx.query["search"];
  if (!search || (search as string).length < 3) {
    ctx.status = statusCodes.BAD_REQUEST;
    ctx.message = !search
      ? "No search keyword in request body/query."
  ...
  if (ctx.state.user.role > 1)
    searchableModels = searchableModels.concat([
      { Ticket: ["title", "trackingId", "detail"] },
      { Page: ["title", "body"] },
    ]);
...
router.post("/", async (ctx) => {
  const searchableModels: { [Model: string]: string[] }[] =
    ctx.state.searchableModels;
  ...
  const searchOutput = await ctx.sequelizeInstance!.transaction(async (t) => {
    ...
    for (const searchModel of searchableModels) {
      ...
      const thisModelResult = await ctx.sequelizeInstance!.models[
        model
      ].findAll({
        where:
          Object.keys(whereFilter).length > 1
            ? {
                [Op.or]: Object.keys(whereFilter).map((filter) => ({
                  [filter]: whereFilter[filter],
                })),
              }
            : whereFilter,
        transaction: t,
      ...
      if (thisModelResult.length)
        output[thisModelResult[0].toJSON()["type"]] = thisModelResult;
    }
    return output;
  });
