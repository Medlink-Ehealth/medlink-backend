...
const changePassword = async (ctx: AppContext, next: Next) => {
  const schema = Joi.object().keys({
    ...
    newPassword: joiPassword
      ...
      .messages({
        "password.minOfUppercase":
          "New Password should contain at least 1 uppercase character",
        "password.minOfSpecialCharacters":
          "New Password should contain at least 1 special character",
        "password.minOfLowercase":
          "New Password should contain at least 1 lowercase character",
        "password.minOfNumeric":
          "New Password should contain at least 1 numeric character",
        "password.noWhiteSpaces": "Password should not contain spaces",
      }),
...
const resetPassword = async (ctx: AppContext, next: Next) => {
  if (!ctx.request.body.email && !ctx.request.body.phoneNumber)
    throwError(
      406,
      "Either email or phone number must be provided to reset password",
    );
...
export const adminFormValidator = {
  createAccount,
  signin,
  updateAccount,
  changePassword,
