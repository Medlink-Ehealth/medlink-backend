  ...
  userSetting.init(
    {
      ...
      sendNotificationsBy: {
        ...
        type:
          sequelize.getDialect()?.toLowerCase() === "postgres"
        ...
        /* 
			...
			*/
        get() {
          const notifications: string[] = this.getDataValue(
            "sendNotificationsBy",
          );
          return notifications.map((key) => ({
            [key]:
              userSettingPlatformProps["sendNotificationsBy"][key as "email"],
          }));
        },
        set(inValues) {
          const possibleValues = Object.keys(
            userSettingPlatformProps["sendNotificationsBy"],
          );

          if (!Array.isArray(inValues))
            throw new Error(
              "notificationsToReceive values should be serializable as array",
            );
          else {
      ...
      notificationsToReceive: {
        type:
          sequelize.getDialect()?.toLowerCase() === "postgres"
            ? DataTypes.ARRAY(DataTypes.STRING)
            : DataTypes.JSON, // same as sendNotificationsBy above
        ...
        set(inValues) {
          const possibleValues = Object.keys(
            userSettingPlatformProps["notificationsToReceive"],
          );
