...
const signAccountInLocal =
  (
    options: {
      ...
      userType?: string;
      signInType?:
        | ((ctx: extendedParameterizedContext) => "email" | "phoneNumber")
        | "email"
        | "phoneNumber";
      accessTokenLifetime?:
        | ((ctx: extendedParameterizedContext) => string | number)
        | string
        | number;
      refreshTokenLifetime?:
        | ((ctx: extendedParameterizedContext) => number)
        | number;
    } | void,
  ) =>
  async (ctx: extendedParameterizedContext, next: Next) => {
    const localSignInType =
      options && options.signInType
        ? typeof options.signInType === "function"
          ? options.signInType(ctx)
          : options.signInType
        : "email";
    ...
    if (options && options.userType)
      ctx.header["x-usertype"] = options.userType;
    // if still nothing
    if (!ctx.header["x-usertype"]) {
    ...
    if (!ctx.sequelizeInstance) {
      logger.error(
        "signAccountInLocal Error: ",
        "No active ctx.sequelizeInstance to match request to!",
      );
    ...
    try {
      if (!check2FAroute) {
        return Passport.authenticate(
          localSignInType === "phoneNumber" ? "phoneNumber" : "local",
          async (err, user) => {
            if (err) {
            ...
            if (!user) {
              ...
              ctx.message =
                localSignInType === "phoneNumber"
                  ? "Oops! Incorrect phone number or password"
              ...
              const userSecurity = await UserSecurity(ctx.sequelizeInstance!)
                .scope("raw")
                .findByPk(user.dataValues.uuid);
              if (userSecurity instanceof UserSecurity) {
                const validators2FA =
                  userSecurity.dataValues["security"] &&
                  userSecurity.dataValues["security"]["2fa"];
                twoFAstatus =
                  validators2FA && validators2FA["verified"] ? true : false;
              }
              // console.log("userSecurity", userSecurity);
              if (!twoFAstatus) {
                //log signin to user signing-in access stream
                const access = await userAccessTimestampsLog(
                  ctx.sequelizeInstance!,
                  {
                ...
                if (accountRoleModelType) {
                  const role = await ctx.sequelizeInstance!.models[
                    accountRoleModelType === "core"
                      ? "Role"
                      : accountRoleModelType
                  ].findByPk(user.dataValues.role);
                  if (role && role.dataValues && role.dataValues["label"])
                    extraData["roleLabel"] = role.dataValues["label"];
                }
                ...
                if (ctx.header["x-requesttoken"]) {
                  ...
                  if (requestToken === "token") {
                    ...
                    if (accessTokenLifetime) {
                      let getTokenLifetime: number | undefined =
                        typeof accessTokenLifetime === "string"
                          ? (!isNaN(Number(accessTokenLifetime)) &&
                              Number(accessTokenLifetime)) ||
                            Number(
                              accessTokenLifetime.substring(
                                0,
                                accessTokenLifetime.length - 1,
                              ),
                            )
                          : accessTokenLifetime;
                      ...
                      if (!isNaN(getTokenLifetime)) {
                        let getTokenLifetimeUnit: string | number | undefined =
                          typeof accessTokenLifetime === "string"
                            ? accessTokenLifetime.split(
                                getTokenLifetime.toString(),
                              )[1]
                            : undefined;
                        ...
                        if (!getTokenLifetimeUnit) {
                          ...
                          accessTokenLifetime =
                            accessTokenLifetime + getTokenLifetimeUnit;
                        }
                        ...
                        getTokenLifetime =
                          getTokenLifetimeUnit &&
                          getTokenLifetime * Number(getTokenLifetimeUnit);
                    ...
                    const token = await encryptionToken(accountData.account, {
                      expiresIn:
                        typeof accessTokenLifetime === "number"
                    ...
                    if (typeof token === "string") {
                      ...
                      if (checkRefreshEnablement) {
                        ...
                        if (typeof refreshToken === "string") {
                          ...
                          const tokenStorage = redis
                            ? redis
                            : config.useCacheAsRedisIsNotAvailable
                              ? Cache
                              : null;
                          if (tokenStorage)
                            tokenStorage.set(
                              `refresh:${accountData.account["uuid" as keyof typeof accountData.account]}`,
                              refreshToken,
                            );
                        }
                      }
                      ...
                      ctx.status = statusCodes.SERVICE_UNAVAILABLE;
                      ctx.message =
                        "Currently unable to generate user access token.";
                      return;
                    }
        ...
        if (decryption && decryption.result) decodedUser = decryption.result;
          ...
          !decryption ||
          (decryption && !decryption["result" as keyof typeof decryption])
        ) {
          ...
          ctx.status = statusCodes.SERVICE_UNAVAILABLE;
          ctx.message =
            "2FA verification currently unavailable. Please try again later";
          return;
        }
        ...
        if (userUUID && userTYPE) {
          ...
          const user = await ctx.sequelizeInstance!.transaction(async (t) => {
            const thisUser = await ctx.sequelizeInstance!.models[
              userTYPE
            ].findOne({ where: { uuid: userUUID }, transaction: t });
            if (thisUser) {
              ...
              if (userSecurity instanceof UserSecurity === false) {
                ctx.status = statusCodes.SERVER_ERROR;
                ctx.message =
                  "Unable to verify the security feature of this account. Please contact an admin";
                return;
              ...
              if (accountRoleModelType) {
                const role = await ctx.sequelizeInstance!.models[
                  accountRoleModelType === "core"
                    ? "Role"
                    : accountRoleModelType
                ].findByPk(thisUser.dataValues.role);
                if (role && role.dataValues && role.dataValues["label"])
                  extraData["roleLabel"] = role.dataValues["label"];
              }
              ...
              if (decodedUser.signInType) {
                ...
                if (requestToken === "token") {
                  ...
                  if (!accessTokenLifetime && decodedUser.accessTokenLifetime)
                    accessTokenLifetime = decodedUser.accessTokenLifetime;
          ...
          ctx.status = statusCodes.NOT_FOUND;
          ctx.message =
            "Oops. We cannot sign you in currently. Please try sign-in later";
          return;
        }
      }
    } catch (err: unknown) {
  ...
  async (ctx: extendedParameterizedContext, next: Next) => {
    ...
    try {
      const user = await ctx
        .sequelizeInstance!.models[userType].scope("raw")
        .findOne({
          where: {
            [Op.or]: [
              {
      ...
      if (user && user instanceof ctx.sequelizeInstance!.models[userType]) {
        ...
        if (accountRoleModelType) {
          const role = await ctx.sequelizeInstance!.models[
            accountRoleModelType === "core" ? "Role" : accountRoleModelType
          ].findByPk(user.dataValues.role);
          if (role && role.dataValues && role.dataValues["label"])
            extraData["roleLabel"] = role.dataValues["label"];
        }
...
const updateAccount =
  ...
  async (ctx: extendedParameterizedContext, next: Next) => {
    ...
    try {
      const user = await ctx.sequelizeInstance!.models[userType].findByPk(
        ctx.state.user.uuid,
      );
      if (user) {
...
const resetAccountPassword =
  ...
  async (ctx: extendedParameterizedContext) => {
    ...
    if (
      (!email && !phoneNumber) ||
      (localSignInType === "email" && !email) ||
      (localSignInType === "phoneNumber" && !phoneNumber)
    ) {
      ctx.status = statusCodes.BAD_REQUEST;
    ...
    try {
      ...
      if (user && user.dataValues[localSignInType]) {
        const linkOrCode = await otpLinkGenerator({
          ...
          numberOfOTPChar:
            options && options.numberOfOTPChar ? options.numberOfOTPChar : 20,
          entityReference: userType,
          queryIdentifier: localSignInType === "email" ? email : phoneNumber,
          route:
            options && options.validationUrl ? options.validationUrl : ctx.path, // optional use current ctx route as dummy path
...
const signAccountInWithThirdParty =
  ...
  async (ctx: extendedParameterizedContext, next: Next) => {
    ...
    if (!ctx.header["x-usertype"]) {
      ...
      logger.error(
        "signAccountInWithThirdParty Error: ",
        "No active ctx.sequelizeInstance to match request to!",
      );
      ctx.status = statusCodes.SERVICE_UNAVAILABLE;
      return;
    ...
    let appName =
      ...
      ctx.params.app ||
      ctx.params.appName ||
      ctx.params.thirdParty; // params to be defined in a variety of ways
    ...
    if (ctx.isAuthenticated()) {
      ...
      const requestToken =
        query["requesttoken"] &&
        (query["requesttoken"] === "session" ||
          query["requesttoken"] === "token")
          ? query["requesttoken"]
          : undefined;
      const callbackUrl =
        options && options.verificationUrl
          ...
          : ctx.path +
            `/verify${requestToken ? "?access=" + requestToken : ""}`; // destination following 3rd party verification for signAccountInWithThirdPartyVerifier to process
  ...
  compose([
    async (ctx: extendedParameterizedContext, next: Next) => {
      ...
      if (!ctx.header["x-usertype"]) {
        ...
        logger.error(
          "signAccountInWithThirdPartyVerifier Error: ",
          "No active ctx.sequelizeInstance to match request to!",
        );
        ctx.status = statusCodes.SERVICE_UNAVAILABLE;
        return;
      ...
      if (ctx.state.redirectQuery) {
        ctx.status = statusCodes.REDIRECTED;
        ctx.redirect(
          (options && options.validationUrl
            ? options.validationUrl
            : "/sign-in-as?") + ctx.state.redirectQuery,
        );
      }
      return;
    },
    async (ctx: extendedParameterizedContext, next: Next) => {
      ...
      const callbackUrl =
        options && options.verificationUrl
          ? options.verificationUrl
          : ctx.path +
            `${localAppSignInType ? "?access=" + localAppSignInType : ""}`;
      ...
      return await Passport.authenticate(
        ...
        async (
          ...
          if (!userTokenisedEmail) {
            ...
            ctx.state.redirectQuery =
              "user=" +
              userTokenisedEmail +
              (localAppSignInType ? "&" + localAppSignInType + "=true" : "");
          }
        },
      )(ctx, next);
    },
...
const signAccountInWithThirdPartyValidateAs =
  ...
  */
    if (ctx.query.user) {
      ...
      const email = decoded
        ? decoded["result" as keyof typeof decoded]
        : undefined;
      ...
      if (!userModelType) {
        ...
        logger.error(
          "signAccountInWithThirdPartyValidateAs Error: ",
          "No active ctx.sequelizeInstance to match request to!",
        );
        ctx.status = statusCodes.SERVICE_UNAVAILABLE;
        return;
      ...
      if (decoded && email) {
        accountData = await ctx.sequelizeInstance!.transaction(async (t) => {
          const user = await ctx.sequelizeInstance!.models[
            userModelType
          ].findOne({
            where: { email: email },
            transaction: t,
          ...
          if (user && user.dataValues.uuid) {
            ...
            let userAccessTimestamp = await UserAccessTimestamp(
              ctx.sequelizeInstance!,
            ).findByPk(account.uuid, { transaction: t });
            if (
              userAccessTimestamp instanceof
              UserAccessTimestamp(ctx.sequelizeInstance!)
            )
              await userAccessTimestamp.update(
                { signedIn: Date.now() },
                { transaction: t },
              );
            //for first time sign-in
            else
              userAccessTimestamp = await UserAccessTimestamp(
                ctx.sequelizeInstance!,
              ).create(
                {
            ...
            if (userAccessTimestamp)
              account["access"] = userAccessTimestamp.toJSON();
            ...
            if (accountRoleModelType) {
              ...
              if (role && role.dataValues && role.dataValues["label"])
                account["roleLabel"] = role.dataValues["label"];
            }
        ...
        ctx.message =
          errorMessage &&
          errorMessage["message" as keyof typeof errorMessage] ===
            "TokenExpiredError"
      ...
      if (accountData && typeof accountData !== "string") {
        ...
        ctx.status = statusCodes.UNAUTHORIZED;
        ctx.message = accountData
          ? accountData
          : "Oops! Currently unable to resolve your account detail.";
        return;
      }
    } else {
      ctx.status = statusCodes.BAD_REQUEST;
...
const createAdminAccount = async (
  ...
  try {
    const newUser = await ctx.sequelizeInstance!.models[
      ctx.state.userType
    ].create({
      ...ctx.request.body,
