...
const resetPassword = async (ctx: AppContext) => {
  ...
  try {
    const user: ClientStatic | null = await Client(ctx.sequelizeInstance)
      .scope("management")
      .findOne({
        where: whereFilter,
      });
    ...
    if (user instanceof Client(ctx.sequelizeInstance)) {
      ...
      return (ctx.body = {
        ...
        statusText:
          "Password reset initiated and reset code sent to user email.",
      });
    }
    ...
    ctx.status = statusCodes.SERVICE_UNAVAILABLE;
    ctx.message =
      "Unable to initiate password reset. Kindly contact site administrator";
    return;
  }
};
