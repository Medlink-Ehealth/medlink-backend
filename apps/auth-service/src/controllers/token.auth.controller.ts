...
const refreshAccessToken =
  ...
  async (ctx: RouterExtendedDefaultContext, next?: Next) => {
    ...
    if (!storage) {
      ...
      ctx.message =
        "Using refresh token is not available on this server becuase no storage is setup to handle this";
      return;
    ...
    let refreshToken =
      tokenOnHeader || ctx.request?.body["refreshToken"]
        ? ctx.request.body["refreshToken"]
        : undefined;

    // check in query
    if (!refreshToken && ctx.query && ctx.query["refreshToken"])
      refreshToken = ctx.query["refreshToken"];
    // check if defined on bearer auth header
    if (!refreshToken)
      refreshToken = ctx.headers.authorization
    ...
    try {
      ...
      if (payload && payload.error) {
        ...
        const accountData = await decryptToken(
          payload["result" as keyof typeof payload] as string,
        );
        ...
        if (
          !accountData ||
          (accountData && !accountData["result" as keyof typeof payload])
        ) {
          ctx.status = statusCodes.BAD_REQUEST;
          ctx.message = "Refresh token is invalid";
          return;
        }
        const data = accountData["result" as keyof typeof payload] as {
          [key: string]: string;
        };
        ...
        if (refreshValidity) {
          refreshValidity = !isNaN(Number(refreshValidity))
            ? refreshValidity + "d"
            : refreshValidity;
