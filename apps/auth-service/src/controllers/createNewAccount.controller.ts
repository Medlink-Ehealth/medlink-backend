...
export const createNewAccount =
  (options?: { verificationExpiry: string | number }) =>
  async (ctx: AppContext, next: Next) => {
    if (!ctx.state.userType) {
      logger.error(
        "User creation error: ",
        "User type needs to be defined in context state: ctx.state.userType",
      );
      ctx.status = statusCodes.BAD_REQUEST;
      ctx.message =
        "Account creation failed as user type undefined in server process.";
      return;
    ...
    try {
      if (ctx.state.userType === "Client") {
        const whereFilter = ctx.request.body.email
        ...
        const checkIfInClient = await Client(ctx.sequelizeInstance).findOne({
          where: whereFilter,
        });
        if (checkIfInClient) {
          ctx.state.error = {
      ...
      if (newUser && newUser.dataValues.uuid) {
        const OTPvalueOrUrl = await otpLinkGenerator({
          ...
          expiry:
            options && options.verificationExpiry
              ? options.verificationExpiry
              : "15m",
        ...
        if (OTPvalueOrUrl) {
          if (
            Array.isArray(OTPvalueOrUrl) &&
            OTPvalueOrUrl[0] === "pendingOtp"
          ) {
            ...
            mailSender({
              ...
              content: {
                ...
                html: newUserAccountCreationTemplate({
                  ...
                  otp: Array.isArray(OTPvalueOrUrl)
                    ? OTPvalueOrUrl[0]
                    : OTPvalueOrUrl,
      ...
      ctx.state.error = {
        ...
        message: (err as object)["parent" as keyof typeof err]
          ? (err as object)["parent" as keyof typeof err][
              "detail" as keyof typeof err
            ]
